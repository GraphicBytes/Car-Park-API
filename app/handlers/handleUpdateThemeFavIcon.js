...
export async function handleUpdateThemeFavIcon(app, req, res) {
  ...
  try {
    ...
    const processingTriggered = await triggerFileProcessing(platform, newFileID, defaultGroupId, 'system_fav_icon');
    ...
    if (processingTriggered) {
      ...
      await platformsModel.collection.updateOne(
        ...
        {
          $set: {
            'data.theme_data.theme_logo.fav_icon': newUrl,
          }
        },
        { upsert: true }
      );
...
export default handleUpdateThemeFavIcon;